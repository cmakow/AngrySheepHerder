...
class GameView {
  constructor(canvas, ctx) {
    ...
    this.retrieveNormalScores = this.retrieveNormalScores.bind(this);
    this.retrieveHardScores = this.retrieveHardScores.bind(this);

    this.scores = [];
    this.lowestHighScore = {score: 0};
    this.highScoreRefHard.once('value').then(this.retrieveHardScores);
    this.highScoreRefEasy.once('value').then(this.retrieveNormalScores);
  }
  ...
  setupGame(moving) {
    ...
    if (moving) {
      ...
      this.highScoreRefHard.on('value', this.retrieveHardScores)
    } else {
      this.newHighScoreRef = this.highScoreRefEasy.push();
      this.highScoreRefEasy.on('value', this.retrieveNormalScores)
    }
  ...
  retrieveNormalScores(snapshot) {
    if (snapshot.val()) {
      this.normalScores = this.retrieveScores(snapshot);
    }
  }

  retrieveHardScores(snapshot) {
    if(snapshot.val()) {
      this.hardScores = this.retrieveScores(snapshot);
    }
  }
  ...
  writeHighScore() {
    const ctx = this.ctx;
    if (this.game.moving) {
      this.lowestHighScore = this.hardScores[9];
    } else {
      this.lowestHighScore = this.normalScores[9];
    }
    if (!this.lowestHighScore || this.game.score > this.lowestHighScore.score) {
  ...
  renderLeaderboards() {
    ...
    this.normalScores.forEach((score, index) => {
